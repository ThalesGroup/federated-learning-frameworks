...
from Mnistvalidator import MnistValidator
...
@pytest.fixture()
def get_mnist_trainer():
    with patch.object(MnistTrainer, "_save_local_model") as mock_save:
        with patch.object(MnistTrainer, "_load_local_model") as mock_load:
            yield MnistTrainer(train_task_name=TRAIN_TASK_NAME, epochs=1)


class TestMnistTrainer:
    @pytest.mark.parametrize("num_rounds", [1, 3])
    def test_execute(self, get_mnist_trainer, num_rounds):
        trainer = get_mnist_trainer
    ...
    @patch.object(MnistTrainer, "_save_local_model")
    @patch.object(MnistTrainer, "_load_local_model")
    def test_execute_rounds(self, mock_save, mock_load):
        train_task_name = "train"
        trainer = MnistTrainer(train_task_name=train_task_name, epochs=2)
        # just take first batch
...
class TestMnistValidator:
    def test_execute(self):
        validate_task_name = "validate"
        validator = MnistValidator(validate_task_name=validate_task_name)
        # just take first batch
